...
#
#for A in /var/spool/asterisk/monitor/q*; do fecha=`date +%Y-%m-%d -r $A`; unique=${A#*-}; uniqueid=${unique%.*}; echo "/usr/local/parselog/update_mix_mixmonitor>

# CONFIGURATION
# You have to set the proper database credentials
$config{'dbhost'} = '10.1.1.29';
